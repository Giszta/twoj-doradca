...
export default function StepCard({ step, index, isActive, onClick }: Props) {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className={`
        relative bg-white rounded-xl shadow-sm transition-all cursor-pointer
        p-4 md:p-5
        ${isActive
    ...
    >
      ...
      {isActive && (
        <div className="absolute inset-0 bg-blue-50 opacity-20 rounded-xl animate-pulse pointer-events-none"></div>
      )}
    </motion.div>
  );
}